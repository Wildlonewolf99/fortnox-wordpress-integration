...
class WF_Plugin {
	...
	public static function print_orders_table_column_content( $column_name, $post_id ){
		...
		print '<a href="#" class="button wallspace-button wallspace-icon-repeat syncOrderToFortnox" data-order-id="' . $order_id . '" data-nonce="' . $nonce . '" title="Sync order to Fortnox"></a> ';

		$synced = self::is_order_synced( $order_id );
		$display = 1 == $synced ? '' : 'wallspace-hidden';

		$order = wc_get_order( $post_id );

		if ( ! $order->meta_exists( 'fortnox_invoice_number'  ) ) {
			$display = "wallspace-hidden";
        }

		print '<a href="#" class="button wallspace-button wallspace-icon-repeat sendInvoiceToCustomer ' . $display . '" data-order-id="' . $order_id . '" data-nonce="' . $nonce . '" title="Send invoice to customer" ><i class="fa fa-send" aria-hidden="true"></i></a>';
        ...
        if( self::order_has_notices( $order_id ) && ! $synced ){
            ...
            print '<span class="wallspace-fortnox-status ' . ( 1 == $synced ? 'wallspace-icon-check' : 'wallspace-icon-cross'  ) . '" title="' . ( 1 == $synced ? __( "Order has synchronized", WF_Plugin::TEXTDOMAIN ) : __( "Order has not synchronized", WF_Plugin::TEXTDOMAIN ) ) . '"></span>';
        }
	...
	public static function print_products_table_column_content( $column_name, $post_id )
	{
		...
		print '<a href="#" class="button wallspace-button wallspace-icon-repeat syncProductToFortnox" data-product-id="' . $post_id . '" data-nonce="' . $nonce . '" title="Sync product to Fortnox"></a> ';

		$synced = self::is_product_synced( $post_id );

		print '<span class="wallspace-fortnox-status ' . ( 1 == $synced ? 'wallspace-icon-check' : 'wallspace-icon-cross'  ) . '" title="' . ( 1 == $synced ? __( "Product has synchronized", WF_Plugin::TEXTDOMAIN ) : __( "Product has not syncronized", WF_Plugin::TEXTDOMAIN ) ) . '"></span>';
		print '<span class="spinner fortnox-spinner"></span>';
	}
