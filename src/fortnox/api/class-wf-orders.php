...
class WF_Orders {
    ...
    public static function format_order_payload( $order_id, $customer_number, $customer )
    {
        ...
        try {
            ...
            if ( has_filter( 'wallspace_fortnox_order'  ) ) {
                wc_deprecated_function( 'The wallspace_fortnox_order filter', '', 'wf_order_payload_before_create_or_update'  );
                $fortnox_order = apply_filters( 'wallspace_fortnox_order', [
                    'CustomerNumber' => $customer_number,
                    'DocumentNumber' => $order_number,
                    'YourOrderNumber' => apply_filters( 'woocommerce_order_number', $order_id, $order ),
                    'ExternalInvoiceReference1' => apply_filters( 'woocommerce_order_number', $order_id, $order ),
                    'OrderDate' => substr( $order->get_date_created(), 0, 10), # To cut off order time
                    'VATIncluded' => apply_filters( 'wallspace_fortnox_sync_order_vat_included', false),
                    'Currency' => $order->get_currency(),
                ], $order );
            }
    ...
    public static function create_order_row( $item, $order, $customer ){
        ...
        if ( has_filter( 'wallspace_fortnox_sync_modify_order_row'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_sync_modify_order_row filter', '', 'wf_order_row_payload_before_create_or_update'  );
            $order_row = apply_filters( 'wallspace_fortnox_sync_modify_order_row', [
                'ArticleNumber'     => WF_Products::sanitized_sku( $product->get_sku() ),
                'Description'       => WF_Products::sanitize_description( $product_name ),
                'DeliveredQuantity' => $item->get_quantity(),
                'OrderedQuantity'   => $item->get_quantity(),
        ...
        if ( has_filter( 'wallspace_fortnox_modify_order_row_sales_account'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_modify_order_row_sales_account filter', '', 'wf_order_row_sales_account'  );
            $account_number = apply_filters( 'wallspace_fortnox_modify_order_row_sales_account', false, $order, $item );
        }
    ...
    public static function get_custom_shipping( $order, $shipping_sku, $customer ){
        ...
        if ( $shipping_product ) {
            ...
            if ( has_filter( 'wallspace_fortnox_shipping_account'  ) ) {
                wc_deprecated_function( 'The wallspace_fortnox_shipping_account filter', '', 'wf_order_shipping_account'  );
                $shipping_account = apply_filters( 'wallspace_fortnox_shipping_account', $shipping_account, $order );
            }
    ...
    public static function set_way_of_delivery( $order, $fortnox_order ){
        ...
        if ( has_filter( 'wallspace_fortnox_way_of_delivery'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_way_of_delivery filter', '', 'wf_order_way_of_delivery'  );
            $fortnox_order['WayOfDelivery'] = apply_filters( 'wallspace_fortnox_way_of_delivery', array_key_exists( 'WayOfDelivery', $fortnox_order ) ? $fortnox_order['WayOfDelivery'] : '', $shipping );
        }
        ...
        if ( has_filter( 'wallspace_fortnox_terms_of_delivery'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_terms_of_delivery filter', '', 'wf_order_terms_of_delivery'  );
            $fortnox_order['TermsOfDelivery'] = apply_filters( 'wallspace_fortnox_terms_of_delivery', array_key_exists( 'TermsOfDelivery', $fortnox_order ) ? $fortnox_order['TermsOfDelivery'] : '' , $shipping );
        }
