...
class WF_Invoices {
    ...
    public static function create_invoice( $order, $order_id ){
        ...
        if ( has_filter( 'wallspace_fortnox_invoice_before_fortnox_submit'  ) ) {
            wc_deprecated_function( 'The filter', '', 'wf_invoice_before_create_or_update'  );
            $invoice = apply_filters( 'wallspace_fortnox_invoice_before_fortnox_submit', $invoice, $order );
        }
    ...
    public static function make_invoice_payment( $order ){
        ...
        if ( has_filter( 'wallspace_fortnox_invoice_payment'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_invoice_payment filter', '', 'wf_invoice_payment_before_create_or_update'  );
            $invoice_payment = apply_filters( 'wallspace_fortnox_invoice_payment', [
                "AmountCurrency"            => $order->get_total(),
                "InvoiceNumber"             => $response->Order->InvoiceReference,
                "PaymentDate"               => substr( $order->get_date_created(), 0, 10 ),
                "ModeOfPaymentAccount"      => self::get_payment_account( $order->get_payment_method() )
