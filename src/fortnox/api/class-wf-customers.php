...
class WF_Customers {
    ...
    public static function format_customer_payload( $order ){
        ...
        if ( has_filter( 'wallspace_fortnox_sync_modify_customer'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_sync_modify_customer filter', '', 'wf_customer_before_post'  );
            $fortnox_customer = apply_filters( 'wallspace_fortnox_sync_modify_customer', [
                'Email'                 => $address['email'],
                'EmailInvoice'          => $address['email'],
                'Name'                  => ! empty( $address['company'] ) ? $address['company'] : $address['first_name'] . ' ' . $address['last_name'],
                'Type'                  => ( ! empty( $address['company'] ) ? "COMPANY" : "PRIVATE" ),
