...
class WF_Products {
	...
	private static function set_fortnox_price( $product, $price_list, $sku ){
        ...
        if ( has_action( 'wallspace_fortnox_after_product_price_update'  ) ) {
            wc_deprecated_function( 'The wallspace_fortnox_after_product_price_update action', '', 'wf_order_after_product_price_update'  );
            do_action( 'wallspace_fortnox_after_product_price_update', $product->get_id(), $sku, $price_list );
        }
        else{
