...
class WF_Admin_Listing_Actions{
    ...
    public static function ajax_flush_access_token(){
        ...
        $refresh_token = get_option('fortnox_refresh_token'); // Replace with the actual refresh token
        ...
        $response = wp_remote_post('https://apps.fortnox.se/oauth-v1/revoke', [
            'headers' => $headers,
            'body' => $body
        ]);
        ...
        if (is_wp_error($response)) {
            return [
                'error' => true,
                'message' => __( "Failed to revoke the refresh token.", 'textdomain' )
            ];
        }
        ...
        if (isset($response_data['revoked']) && $response_data['revoked'] === true) {
            return [
                'error' => false,
                'message' => __( "Refresh token has been removed.", 'textdomain' )
            ];
        } else {
            return [
                'error' => true,
    ...
    /** Authorize with Fortnox to get Authorization Key
     *
     */
    /*public static function ajax_authorize_with_fortnex() {
        $client_id = get_option('fortnox_client_id');
        $redirect_uri = admin_url('admin-post.php?action=fortnox_callback');
        $state = 'penang';
        $scope = 'customer order invoice article companyinformation';
        ...
        $response = wp_remote_get($authorize_url);

        if (is_wp_error($response)) {
            return [
                'error' => true,
                'message' => 'Failed to authorize with Fortnox.',
            ];
        }
        ...
        if (!isset($data['code'])) {
            return [
                'error' => true,
                'message' => json_encode($response)//'Authorization code not received.',
            ];
        }       
    
        $code = $data['code'];
        $authorization_code = sanitize_text_field($code);
        ...
        if (isset($token_data['access_token'])) {
            update_option('fortnox_access_token_oauth2', $token_data['access_token']);
            update_option('fortnox_refresh_token', $token_data['refresh_token']);
            update_option('fortnox_access_token_expiry_time', time() + $token_data['expires_in']);
            return [
                'error' => false,
